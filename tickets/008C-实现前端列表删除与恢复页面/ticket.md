...
### 包含
- 列表删除入口复用 008A 侧边栏自定义列表的"列表设置"菜单中的"删除"项
- 删除二次确认弹窗
- 调用 DELETE `/api/lists/{id}` 并携带 `expectedVersion`
- 删除成功后立即从侧边栏隐藏该列表
- 删除成功后显示固定时间 toast，提供"撤销"按钮
- 点击"撤销"调用 POST `/api/lists/{id}/restore` 并携带 `expectedVersion` 恢复列表
- 409 版本冲突时提示用户刷新后重试

### 不包含
- 硬删除与 cleanup Lambda 触发逻辑（后端 018）
- 撤销期限检查与 `410 GONE` 处理（本期不实现）
...
- 覆盖删除确认、删除成功、删除隐藏、撤销成功、撤销超时、409 冲突
