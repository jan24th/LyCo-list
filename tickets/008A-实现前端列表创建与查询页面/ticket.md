...
- 创建成功后刷新列表查询缓存
- 列表项展示名称、颜色
- 自定义列表项提供"列表设置"入口（按钮或右键菜单），菜单内含"编辑"和"删除"选项，供 008B/008C 复用

### 不包含
- 列表编辑表单与 API 调用（008B）
- 列表删除/恢复弹窗与 API 调用（008C）
- 列表手动拖拽排序
- 列表详情页面内的任务 CRUD
- 列表分享与权限
...
Given 已认证用户
When 将鼠标悬停或聚焦到自定义列表项
Then 可见"列表设置"入口，点击后弹出包含"编辑""删除"选项的菜单
