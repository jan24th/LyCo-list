...
### 2026-07-15 Triage: wontfix（already implemented）

该功能已在代码库中完整实现，ticket 状态未回写。实现位置：

- Repository：`apps/api/src/lists/db.ts`（createList / queryActiveLists / updateList / deleteList / restoreList）
- Handler：`apps/api/src/lists/index.ts`（5 条路由，409/404/400 错误映射）
- SST 路由：`sst.config.ts`（LycoTable + GSI1 + JWT 授权）
- Bruno：`bruno/lyco-list/lists/`（5 个请求文件）
- 测试：`bunx vitest run apps/api/src/lists` — 51/51 通过
- 提交：`69b8fc1`、`6dd6eda`、`f76b4cd`、`d51102e`、`95d6c8a`、`bbbfcde`

七条验收标准逐条核对全部满足，故以 already-implemented 关闭。
