...
作为用户，我希望修改自定义列表的名称和颜色，以便让列表更符合我的分类习惯。
...
- `.scratch/lists/issues/02-实现前端列表创建与查询页面.md` 列表查询与创建基础
...
**结论**：Plan 结构可执行，按下列裁定修正后实施。ticket 正文已同步更新（icon 移除、关联路径）。

**裁定 1（icon 残留）**：ticket 与 plan 示例中的 icon 字段全部作废——表单只有名称 + 颜色，提交载荷 `{name, color, expectedVersion}`（`ListUpdate` 已无 icon）。

**裁定 2（测试命令/导入）**：`bun run test -- --project apps/web <path>` 改为 `bunx vitest run <path>`；import 不带 `.js` 扩展。

**裁定 3（Dialog 测试）**：完全 mock `useUpdateListMutation` 后用普通 `render`（`renderWithQuery` 是 hook 渲染器，不用于组件）。

**裁定 4（实现细节）**：
- Sidebar 渲染 `EditListDialog` 时加 `key={editingList.id}`，切换编辑目标时强制重置表单状态
- 弹窗挂在 Sidebar 组件末尾（无 `</aside>`，Sidebar 是纯导航组件）

**核实无问题**：`ApiError` 增强必要且正确；invalidate 模式与 008A 一致；`onEdit={setEditingList}` 签名吻合；阻塞项 02 已 resolved。

### 2026-07-15 Agent Brief

**1. Mission**：从 Sidebar 列表设置菜单进入编辑弹窗，PATCH `/api/lists/{id}` 携带 `expectedVersion`，成功后刷新缓存，409 提示“数据已过期，请刷新后重试”。

**2. Constraints**：web-only；TDD + 四项覆盖率 100%；复用 `@lyco/shared` 的 `List`/`ListUpdate`；无 icon 字段。

**3. Execution plan**：Task 1（ApiError，TDD）→ Task 2（updateList + 409 转换，TDD）→ Task 3（useUpdateListMutation，TDD）→ Task 4（EditListDialog 名称+颜色，TDD）→ Task 5（Sidebar 接线 + key 重置，TDD）→ 全量验证 + 浏览器实测。

**4. Verification**：`bun run test` 全绿且覆盖率阈值通过；`bunx tsc --noEmit -p apps/web/tsconfig.app.json`；`bunx @biomejs/biome check .`；`bun run --cwd apps/web build`；浏览器实测编辑名称/颜色全流程（本地 API stub 支持 PATCH）。

**5. Done criteria**：4 个验收场景全过；`apiClient` 既有行为（401 重定向）无回归；编辑成功后 Sidebar 即时刷新。

**6. Watch out**：
- 既有 `api.test.ts` 的非 2xx 断言可能依赖旧 error message，改 ApiError 时同步调整
- `updateList` 只在 `error instanceof ApiError && status === 409` 时转换消息，其余错误原样抛出
- mock 的 mutationFn 会收到第二个 context 参数（react-query v5），断言用 `expect.anything()`

### 2026-07-15 完成记录

- 实现提交：`88c726f`（ApiError + updateList）、`8e376fc`（测试修复）、`ec82375`（hook + EditListDialog + Sidebar 接线）
- 全套 311 个测试通过，四项覆盖率 100%；tsc、Biome、production build 通过
- 浏览器验收通过（本地 API stub + Chrome 实测）：编辑名称即时刷新侧边栏；用 curl 制造并发 version 后实测 409 → 显示“数据已过期，请刷新后重试”
- 双轴 code review 通过，无返工项
- 过程记录：`88c726f` 曾因管道遮蔽 vitest exit code 带上失败测试，`8e376fc` 立即修复
- 008C 集成约束：为 `ListSettingsMenu` 的 `onDelete` 挂接删除确认弹窗；删除/恢复后 invalidate `LISTS_QUERY_KEY`
