...
- `.scratch/lists/issues/01-实现列表crud软删除和恢复.md` 后端删除/恢复 API 规范
- `.scratch/lists/issues/02-实现前端列表创建与查询页面.md` 列表查询与侧边栏基础
...
### Triage 裁定（2026-07-15）

> *This was generated by AI during triage.*

1. 解除阻塞：`Blocked by: 02`（008A）已 resolved
2. 契约已与后端核对：DELETE 用 query param `?expectedVersion=x`，restore 用 POST body `{expectedVersion}`（`apps/api/src/lists/index.ts` L96/107）——plan 与之吻合
3. 测试夹具残留 `icon: "list"` → 删除，fixture 与现状对齐
4. 测试命令统一为 `bunx vitest run <path>`；导入不带 `.js` 扩展名
5. `renderWithQuery` 是 hook 渲染器，不能用于组件渲染；hook 全 mock 后用普通 `render`；Radix 菜单交互用 userEvent
6. Task 5 的 Sidebar 片段是旧结构（`<aside>` 容器）→ 仅在现有纯导航 Sidebar 上增量接线：`onDelete={setDeletingList}` + 挂 `DeleteListDialog`
7. 不导出 `buildDeleteToastConfig` 纯函数——直接全链路测：mock `sonner` 后从 `toast.success` 调用参数取 `action.onClick()` 触发撤销
8. 撤销失败需可见：restore mutate 加 `onError` → `toast.error(错误消息)`（409 提示同时适用删除与恢复）
9. 删除按钮用 `variant="destructive"`（button.tsx 已有该 variant）；错误提示用 `role="alert"` + `text-destructive`
10. 删除覆盖修复时加的临时测试 “keeps the list when delete is clicked before 008C is wired”，替换为“打开删除确认弹窗”测试
11. 场景 2“任务不再显示”：任务 UI 尚未存在，列表缓存失效即满足；后续 012+ 验证
12. `main.tsx` 与 `components/ui/**` 已在 coverage exclude 中，挂 `<Toaster />` 无覆盖率风险

## Agent Brief

### Mission
实现前端列表删除与恢复：删除二次确认弹窗 → DELETE 携带 `expectedVersion` → 成功后缓存失效隐藏列表并显示带“撤销”按钮的 toast（固定时长）→ 撤销调用 POST restore 恢复列表。删除与恢复的 409 均提示“数据已过期，请刷新后重试”。

### Constraints
- React 19 + TypeScript + TanStack Query + shadcn/ui + sonner
- 纯前端改动：`apps/web`
- TDD：先写失败测试；测试命令 `bunx vitest run <path>`；全量验证看 exit code（禁止管道遮蔽）
- 导入不带 `.js` 扩展名；约定式提交
- Sidebar 保持纯导航组件，仅增量接线

### Execution plan
1. `lib/lists.ts` 加 `deleteList` / `restoreList`（409 转换，复用 `ApiError`）→ 验证：lists.test.ts 新增 4 测全过
2. `hooks/use-lists.ts` 加 `useDeleteListMutation` / `useRestoreListMutation`（onSuccess invalidate）→ 验证：use-lists.test.tsx 新增 2 测全过
3. `main.tsx` 挂 `<Toaster />`（ui/sonner.tsx 已存在）→ 验证：tsc 通过
4. 新建 `DeleteListDialog`：确认/取消、destructive 按钮、pending 禁用、`role="alert"` 错误展示、成功后 `onDeleted(deletedList)` 回调 → 验证：组件测试全过（含取消、409、pending 分支）
5. Sidebar 接线：`onDelete={setDeletingList}` + 挂弹窗；`handleDeleted` 发 `toast.success("「name」已删除", { duration: 5000, action: { label: "撤销", onClick } })`；onClick 调 restore mutate（`expectedVersion` 用删除返回的 version，`onError` → `toast.error`）→ 验证：Sidebar 测试全过（打开确认弹窗、全链路删除+toast、撤销触发、撤销失败 toast）
6. 浏览器实测（本地 stub 加 DELETE/restore 路由）：删除确认 → 列表消失 → toast 撤销 → 列表恢复；409 冲突提示
7. 全量验证：退出码 0、覆盖率 100%、`bunx @biomejs/biome check --write` 后 `biome ci` 干净、tsc、production build

### Done criteria
- 全套测试退出码 0，覆盖率 100%；Biome ci / tsc / build 干净
- 浏览器实测删除-撤销闭环与 409 提示

### Watch out
- 撤销的 `expectedVersion` 用删除响应返回的最新 version（后端删除时递增）
- Radix 弹窗 aria-hidden：弹窗内查询用 `within(dialog)`
- 提交前必须 `biome check --write`（手写代码易留格式问题，CI Lint 是硬门槛）
- 别改 `components/ui/**`（shadcn 生成物）

### 2026-07-15 完成记录

- 实现提交：`f3033b8`（API wrappers）、`704fe46`（hooks）、`b4813af`（DeleteListDialog）、`fd11329`（Sidebar 接线）、`6819b1a`（同步脚本格式化）
- `<Toaster />` 在 main.tsx 中早已挂载，Task 3 无需改动
- 全套 330 个测试 exit 0，覆盖率 100%；tsc、Biome ci、production build 通过
- 浏览器验收通过（本地 stub 加 DELETE/restore 路由 + Chrome 实测）：二次确认弹窗 → 删除后列表立即消失 → toast 102ms 内出现（MCP 往返超 5s 会错过 toast，用轮询脚本验证）→ 撤销后列表恢复
- 双轴 code review 通过
- 集成约束留给后续：任务列表 UI（012+）需验证“删除列表后其任务不再显示”；`<Toaster />` 已全局可用，其他功能可直接 `toast`
