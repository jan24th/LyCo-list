...
Estimate: 3
Blocked by: ../../web-shell/issues/03-实现响应式应用壳与移动端导航骨架.md, ../../auth/issues/02-实现cognito登录回调与401重定向处理.md, 01
PHASE: 1
CYCLE: 3
...
**结论**：Plan 结构可执行，但需要按下列裁定修正后再实施。

**核实结果**
- ✅ 智能列表定义与设计文档一致；`apiClient` 签名、`ListInput`/`List` 类型假设均与现状吻合
- ✅ 阻塞项（004A、005、008）全部 resolved

**裁定 1（硬冲突）：Task 8 不得重建布局。** Plan 中重写 `App.tsx` 为自带 `<aside>` 的 flex 布局会摧毁 004A 交付的 AppShell（移动端 Sheet、安全区域、断点切换）。裁定：
- `Sidebar` 是**纯导航内容组件**：智能列表区 + "我的列表"区 + 新建列表按钮；不含 `<aside>` 容器、不含 LyCo-list 标题（AppShell 已承载）
- `App.tsx` 仅将 `navigation={<Navigation />}` 替换为 `navigation={<Sidebar />}`，删除临时 `Navigation` 函数，其余（staticData 标题机制、AppShell 使用）不动
- Sidebar 内链接用普通 `<a>` 或 TanStack `<Link>` 均可（AppShell 的 closeAfterNavigation 委托对两者都生效，移动端 Sheet 会自动关闭）
- 连带修改：`apps/web/src/routes/about.test.tsx` 中点击"首页"链接的壳集成测试，需改为点击 Sidebar 内链接（注意 004A 已有 header h1，查询要精确）

**裁定 2（Task 1 缩减）**：`lucide-react`、`sonner` 已安装；`dialog/dropdown-menu/input/label/sonner` shadcn 组件已全部存在。Task 1 只执行：
```bash
bun add @tanstack/react-query --cwd apps/web --registry https://registry.npmmirror.com
```

**裁定 3（测试命令）**：所有 `bun run test -- --project apps/web <path>` 改为 `bunx vitest run <path>`（vitest 3 按项目 name 匹配，web 未命名）。

**小修正**
- `ListSettingsMenu.test.tsx` 示例代码缺 `render` 导入，需补上
- web 端 import 不带 `.js` 扩展（与 `App.tsx` 现状一致）；plan 中 `@/lib/api.js` 等写法需去掉扩展名
- `main.tsx` 包裹 `QueryClientProvider` 时保留现有 `ThemeProvider` / `Toaster` 嵌套
- Task 9 的 typecheck 用 `bunx tsc --noEmit -p apps/web/tsconfig.app.json` 即可（其余项目未被本 ticket 触碰）

### 2026-07-15 Agent Brief

**1. Mission**：侧边栏展示 6 个智能列表入口 + 自定义列表（名称/颜色/设置入口），"新建列表"弹窗创建并刷新缓存。Sidebar 作为纯导航内容组件接入 004A 的 AppShell `navigation` 插槽，不触碰布局。

**2. Constraints**：web-only 变更；复用 `@lyco/shared` 类型与现有 `apiClient`；TDD + 四项覆盖率 100%；测试命令 `bunx vitest run <path>`；约定式提交。

**3. Execution plan**：按原 Plan Task 1（缩减版，仅装 react-query）→ Task 2（QueryClient + test-utils + main.tsx）→ Task 3（lib/lists）→ Task 4（use-lists hooks）→ Task 6（ListSettingsMenu）→ Task 7（NewListDialog）→ Task 5（Sidebar，改为纯导航组件，无 aside）→ Task 8'（App.tsx 换 navigation 插槽 + 修 004A 壳测试）→ Task 9 全量验证。建议 Task 5 在 6/7 之后做（原顺序 Sidebar 依赖未实现组件）。

**4. Verification**：`bun run test` 全绿且覆盖率阈值通过；`bunx tsc --noEmit -p apps/web/tsconfig.app.json`；`bunx @biomejs/biome check .`；`bun run --cwd apps/web build`；浏览器实测桌面/移动两视口下 Sidebar 渲染、Sheet 内点击链接自动关闭、新建列表全流程。

**5. Done criteria**：5 个验收场景全过；AppShell 能力（Sheet、安全区域、断点、标题机制）无回归；008B/008C 可通过 `ListSettingsMenu` 的 `onEdit`/`onDelete` 回调挂接。

**6. Watch out**：
- Task 8' 会打破 004A 在 `about.test.tsx` / `index.test.tsx` 里依赖"首页/关于"链接的测试，须同步修正（这是预期内的测试维护，不是回退）
- Sidebar 中"今天"与 AppShell header h1 可能同文案，测试查询用 `within` 或 role 精确化
- `fetchLists` 的分页（nextCursor）本期不做 UI，但 API 封装保留 cursor 参数
- `ListInput` 的 color/icon/order 有后端默认值，前端表单仍按 plan 显式提交

### 2026-07-15 补充裁定：移除 icon 字段

> *This was generated by AI during triage.*

经用户确认，采纳旧分支 `feat/008a-list-sidebar` 上的设计变更：**List 不再包含 `icon` 字段**。共享 schema、API fixtures、Bruno、设计文档已同步移除；新建列表表单只收集名称与颜色。issue 正文与验收场景已更新，旧 Plan 示例代码中的 `icon` 相关片段作废。

### 2026-07-15 完成记录

- 实现提交：`f98e616`（icon 移除）、`a073c8b`（数据层）、`7f1e773`（导航组件）、`94c7558`（AppShell 集成）
- 全套 297 个测试通过，四项覆盖率 100%；tsc、Biome、production build 通过
- 浏览器验收通过（本地 API stub + Chrome 实测）：桌面/移动 Sidebar 渲染、创建列表即时出现、Sheet 点击链接自动关闭
- 双轴 code review 通过，无返工项
- 旧分支 `feat/008a-list-sidebar` 的早期实现按用户决定弃用（其 App 布局与 AppShell 冲突），仅采纳其中的 icon 移除设计变更
- 008B/008C 集成约束：在 `Sidebar` 内为 `ListSettingsMenu` 的 `onEdit`/`onDelete` 回调挂接编辑/删除弹窗；编辑/删除后 invalidate `LISTS_QUERY_KEY`
